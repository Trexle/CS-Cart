...
{if $payment_method.processor_params.test == 'Y'}
    {script src="https://cdn.trexle.com/trexle.js"}
{else}
    {script src="https://cdn.trexle.com/trexle.js"}
{/if}
...
<div class="clearfix">
    <div id="hidden-fields">
        <input type="hidden" name="trexle" id="trexle" value="true">
    </div>
...
(function(_, $) {
    $(document).ready(function() {
        ...
        form = $('#trexle').closest('form');
        form.addClass("trexle");
        ...
        // Set Publishable Key
        Trexle.setPublishableKey("{$payment_method.processor_params.publishable_key}");
        ...
        submit.click(function(e){
            ...
            {rdelim};
            // Create token and handle response from Trexle
            Trexle.createToken(card, trexleResponse);
        });
        
        // Handle Response from Trexle
        function trexleResponse(response){
            ...
            if (response.response) {
                ...
                loading.hide();
                submitContainer.append('<div class="trexle-error trexle-description">'+response.error_description+'</div>');
                if (response.messages) { 
                    $.each(response.messages, function(i, m){ 
                        $('input[id^="cc_'+m.param+'"]').addClass('cm-failed-field');
                        $('input[id^="cc_'+m.param+'"]').parent().append('<div class="trexle">'+m.message+'</div>');
                    });
                }
                // re-enable submit
